...
import { MessageSquare, X, Send, Bot, User, Loader2, Copy, Check, Trash2 } from "lucide-react";
import { Button } from "../UI/Button";
import { useLocalStorage } from "../../hooks/useLocalStorage";

export const ChatPanel = ({ isOpen, onClose, vaultContext = "" }) => {
  const [messages, setMessages] = useLocalStorage("matriks-chat-history", []);
  ...
  return (
    <div className="fixed right-4 bottom-4 z-40 flex h-[600px] w-full max-w-md flex-col rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900 animate-in slide-in-from-bottom-4 fade-in duration-200">
      <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3 dark:border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Matriks AI</h3>
            <p className="text-[10px] text-slate-500 dark:text-slate-400">Powered by Kimi</p>
          </div>
        </div>
      ...
      <div className="border-t border-slate-100 p-3 dark:border-slate-800">
        <div className="flex items-end gap-2 rounded-xl border border-slate-200 bg-white p-2 dark:border-slate-700 dark:bg-slate-900">
          <textarea
            ref={inputRef}
            value={input}
